...
import SpinnerBaseSquareHorizontal from "../loadingSpins";
...
export default function HospitalSignUpForm() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      await signUp(email, password, { role: "hospital", hospitalName });
      toast.success(
        "Sign up successful! Please check your email to verify your account and then login",
      );
      router.push("/");
    } catch (error: unknown) {
    ...
    <>
      <form onSubmit={handleSignUp} className="flex flex-col gap-4">
        <div className="flex flex-col">
          ...
          <input
            type="text"
            placeholder="eg: City General Hospital"
